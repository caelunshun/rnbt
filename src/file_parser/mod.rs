...
impl FileParser {
    ...
    fn parse_bytes(&self, bytes: &[u8]) -> Result<NbtTag, ()> {
        ...
        let root = self.parse_compound(&mut cursor, name)?;
    
        Ok(NbtTag::Compound(root))
    }
    
    fn parse_compound(&self, cursor: &mut Cursor<&[u8]>, name: String) -> Result<NbtTagCompound, ()> {
        ...
        loop {
            ...
            // Read value
            let value = self.parse_value(cursor, ty, name.clone())?;
    ...
    fn parse_list(&self, cursor: &mut Cursor<&[u8]>, name: String) -> Result<NbtTagList, ()> {
        ...
        for _ in 0..len {
            let val = self.parse_value(cursor, ty, "".to_string())?;
            values.push(val)
        }
    ...
    fn parse_value(&self, cursor: &mut Cursor<&[u8]>, ty: NbtTagType, name: String) -> Result<NbtTag, ()> {
        Ok(match ty {
            ...
            NbtTagType::List => {
                let list = self.parse_list(cursor, name)?;
                NbtTag::List(list)
            }
            NbtTagType::Compound => {
                let compound = self.parse_compound(cursor, name)?;
